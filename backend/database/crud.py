...
def create_dark_store(db: Session, name: str, lat: float, lon: float, radius_km: float = 5.0, current_zone: str = "GREEN") -> DarkStore:
    ds = DarkStore(
        name=name, lat=lat, lon=lon, radius_km=radius_km, current_zone=current_zone,
...
def reset_all_dark_stores(db: Session):
    # 1. Reset physical hubs back to safety (GREEN)
    db.query(DarkStore).update({"current_zone": "GREEN"})
    
    # 2. Reset all active riders to baseline state (GREEN)
    db.query(Rider).update({"zone_category": "GREEN"})
    
    # 3. Commit changes (History tables like ClaimLedger are preserved)
    db.commit()
...
def create_rider(db: Session, name: str, phone: str,
    ...
    rider = Rider(
        name=name, phone=clean_phone, hub_name=hub_name,
...
def get_rider_by_phone(db: Session, phone: str) -> Optional[Rider]:
    clean_phone = "".join(filter(str.isdigit, phone))
    return db.query(Rider).filter(Rider.phone.contains(clean_phone[-10:])).first()
...
def update_rider_zone(db: Session, rider_id: int, zone_category: str) -> Optional[Rider]:
    rider = get_rider(db, rider_id)
    if not rider:
        return None
    rider.zone_category = zone_category.upper()
    db.commit()
    db.refresh(rider)
    return rider
...
def update_rider_gps(db: Session, rider_id: int, lat: float, lon: float) -> Optional[Rider]:
    ...
    if rider:
        rider.lat = lat
        rider.lon = lon
        rider.geom = func.ST_SetSRID(func.ST_MakePoint(lon, lat), 4326)
        db.commit()
        db.refresh(rider)
    return rider
...
def create_trigger_event(db: Session, source: str, trigger_type: str,
    ...
    event = TriggerEvent(
        ...
        geom=func.ST_SetSRID(func.ST_MakePoint(center_lon, center_lat), 4326)
    )
    db.add(event)
    db.commit()
    db.refresh(event)
